...
export function ImageUpload({
  ...
  const [pendingCrop, setPendingCrop] = useState<{ src: string; mimeType: string } | null>(null);
  ...
  function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setSizeError(null);
    if (crop) {
      setPendingCrop({ src: URL.createObjectURL(file), mimeType: file.type });
    } else {
  ...
  function handleCrop(croppedFile: File) {
    ...
    if (state.type === "new") URL.revokeObjectURL(state.previewUrl);
    onChange({ type: "new", file: croppedFile, previewUrl: URL.createObjectURL(croppedFile) });
    setPendingCrop(null);
  }
